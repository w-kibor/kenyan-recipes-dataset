...
# Fetch already processed if the CSV output exists to resume
existing_paths = {}
    ...
    if 'image_path' in existing_df.columns:
        for idx, row in existing_df.iterrows():
            if pd.notna(row['image_path']) and row['image_path'] != "":
                existing_paths[row['title']] = row['image_path']
...
for index, row in df.iterrows():
    ...
    if os.path.exists(image_filepath):
        print(f"[{index+1}/{len(df)}] {title[:30]}... -> Exists")
        image_paths.append(relative_path)
        continue

    # Resume from previous runs if available
    if title in existing_paths and existing_paths[title] != "":
        if os.path.exists(existing_paths[title]):
            print(f"[{index+1}/{len(df)}] {title[:30]}... -> Restored from CSV")
            image_paths.append(existing_paths[title])
            continue
    ...
    query = f"Kenyan food {clean_query}"
    ...
    if image_url:
        ...
        # Fallback query
        query_fallback = clean_query
        image_url_fallback = fetch_image_url(query_fallback)
        if image_url_fallback:
            success = download_image(image_url_fallback, image_filepath)
            if success:
                print(f"  -> Saved (fallback): {image_filename}")
                image_paths.append(relative_path)
                time.sleep(1)
            else:
                image_paths.append("")
